...
function JerseyAvatar({ jersey }: { jersey: string }) {
  return (
    <div className="w-8 h-8 rounded-full bg-[#030b14] border border-[#009EE0]/30 flex items-center justify-center shrink-0" aria-hidden="true">
  ...
  return (
    <div className="flex items-center gap-2 p-2 bg-[#091627]/80 rounded-xl">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="text-xs text-[#009EE0]/50">{p.position}</div>
      </div>
      <div className="text-right shrink-0">
        <div className="text-base font-bold text-[#009EE0]" style={{ textShadow: '0 0 8px rgba(0,158,224,0.5)' }}>{statValue}</div>
        <div className="text-xs text-[#009EE0]/50">{statLabel}</div>
      </div>
    </div>
  )
}
...
export default function SquadSection({ squad }: { squad: Player[] }) {
  ...
  return (
    <section className="bg-[#060f1a]/96 backdrop-blur-sm rounded-xl border border-[#009EE0]/15 p-4" style={{ boxShadow: '0 0 25px rgba(0,158,224,0.05)' }} aria-label="Squad stats">
      <div className="flex items-start justify-between mb-4 gap-4">
        <h2 className="font-bold text-lg text-[#009EE0] uppercase tracking-[0.08em]" style={{ textShadow: '0 0 12px rgba(0,158,224,0.4)' }}>Squad Stats</h2>
        <span className="text-xs text-[#009EE0]/50 italic text-right">* Season totals — may include goals from previous clubs</span>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div>
          ...
          {topApps.length === 0 ? (
            <p className="text-[#009EE0]/50 text-sm">No data</p>
          ) : (
      ...
      <p className="text-xs text-[#009EE0]/50 mt-4">
